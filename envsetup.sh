...
EOF
    ...
    local A
    A=""
    for i in `cat $T/build/envsetup.sh | gsed -n "/^function /s/function \([a-z_]*\).*/\1/p" | sort`; do
      A="$A $i"
    done
    echo $A
}
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cm_") ; then
       CM_BUILD=$(echo -n $1 | gsed -e 's/^cm_//g')
       NAM_VARIANT=$(echo -n $1 | gsed -e 's/^cm_//g')
...
function lunch()
{
    ...
    local product=$(echo -n $selection | gsed -e "s/-.*$//")
    ...
    local variant=$(echo -n $selection | gsed -e "s/^[^\-]*-//")
...
{
    if [ "$OUT" ] ; then
        MODVERSION=`gsed -n -e'/ro\.cm\.version/s/.*=//p' $OUT/system/build.prop`
...
function pid()
{
   local EXE="$1"
   if [ "$EXE" ] ; then
       local PID=`adb shell ps | fgrep $1 | gsed -e 's/[^ ]* *\([0-9]*\).*/\1/'`
       echo "$PID"
   else
       echo "usage: pid name"
   fi
...
function godir () {
    ...
    local lines
    lines=($(grep "$1" $T/filelist | gsed -e 's/\/[^/]*$//' | sort | uniq))
    if [[ ${#lines[@]} = 0 ]]; then
        echo "Not found"
        return
    fi
...
function cmremote()
{
    ...
    GERRIT_REMOTE=$(cat .git/config  | grep git://github.com | awk '{ print $NF }' | gsed s#git://github.com/##g)
    if [ -z "$GERRIT_REMOTE" ]
    then
        GERRIT_REMOTE=$(cat .git/config  | grep http://github.com | awk '{ print $NF }' | gsed s#http://github.com/##g)
        if [ -z "$GERRIT_REMOTE" ]
        then
...
function makerecipe() {
  ...
  cd ..
  cd android
  gsed -i s/'default revision=.*'/'default revision="refs\/heads\/'$1'"'/ default.xml
  git commit -a -m "$1"
  cd ..
...
function cmgerrit() {
    ...
    local user=`git config --get review.review.pecancm.insomnia247.nl.username`
...
function cmrebase() {
    ...
    cd $dir
    repo=$(cat .git/config  | grep git://github.com | awk '{ print $NF }' | gsed s#git://github.com/##g)
